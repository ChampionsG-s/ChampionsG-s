'use client'

import { useState } from 'react'
import { useRouter } from 'next/navigation'
import { X, Swords } from 'lucide-react'
import { createClient } from '@/lib/supabase/client'
import { rankValue } from '@/lib/duels'
import { DeckStack, PlayingCardFlip } from './duel-card-visuals'
import { cn } from '@/lib/utils'
import type { Duel } from '@/types'

interface DuelDrawModalProps {
  duel: Duel
  currentUserId: string
  usersMap: Map<string, { username: string; avatar_url?: string | null }>
  onClose: () => void
}

// Ventana para sacar TUS cartas de un duelo ya aceptado. El primer click
// en "Sacar carta 1" dispara el pedido al servidor (draw_duel_cards, que
// reparte tus 2 cartas de una vez); el segundo click solo revela la
// segunda card ya obtenida. Si el rival todavia no saco las suyas, el
// duelo queda "esperando"; si con esto se completan las 4, se resuelve al
// toque y tambien se revelan las del rival.
export function DuelDrawModal({ duel: initialDuel, currentUserId, usersMap, onClose }: DuelDrawModalProps) {
  const supabase = createClient()
  const router = useRouter()
  const [duel, setDuel] = useState(initialDuel)
  const [drawing, setDrawing] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [myRevealed, setMyRevealed] = useState(0)
  const [rivalRevealed, setRivalRevealed] = useState(0)

  const isChallenger = duel.challenger_id === currentUserId
  const rivalId = isChallenger ? duel.opponent_id : duel.challenger_id
  const rivalName = usersMap.get(rivalId)?.username ?? 'Rival'

  const myCard1Rank = isChallenger ? duel.challenger_card1_rank : duel.opponent_card1_rank
  const myCard1Suit = isChallenger ? duel.challenger_card1_suit : duel.opponent_card1_suit
  const myCard2Rank = isChallenger ? duel.challenger_card2_rank : duel.opponent_card2_rank
  const myCard2Suit = isChallenger ? duel.challenger_card2_suit : duel.opponent_card2_suit
  const rivalCard1Rank = isChallenger ? duel.opponent_card1_rank : duel.challenger_card1_rank
  const rivalCard1Suit = isChallenger ? duel.opponent_card1_suit : duel.challenger_card1_suit
  const rivalCard2Rank = isChallenger ? duel.opponent_card2_rank : duel.challenger_card2_rank
  const rivalCard2Suit = isChallenger ? duel.opponent_card2_suit : duel.challenger_card2_suit

  const iHaveDrawn = myCard1Rank !== null
  const rivalHasDrawn = rivalCard1Rank !== null
  const isResolved = duel.status === 'resolved'
  const myFullyRevealed = myRevealed >= 2
  const rivalFullyRevealed = rivalRevealed >= 2

  const myTotal = iHaveDrawn ? rankValue(myCard1Rank!) + rankValue(myCard2Rank!) : null
  const rivalTotal = rivalHasDrawn ? rankValue(rivalCard1Rank!) + rankValue(rivalCard2Rank!) : null
  const isTie = duel.winner_id === null
  const iWon = duel.winner_id === currentUserId

  const handleDraw = async () => {
    setDrawing(true)
    setError(null)
    const { data, error: rpcError } = await supabase.rpc('draw_duel_cards', { target_duel: duel.id })
    setDrawing(false)
    if (rpcError) {
      setError(rpcError.message)
      return
    }
    setDuel(data as Duel)
    setMyRevealed(1)
    router.refresh()
  }

  const handleClose = () => {
    router.refresh()
    onClose()
  }

  return (
    <div
      className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm"
      onClick={handleClose}
    >
      <div
        className="w-full sm:max-w-sm rounded-t-2xl sm:rounded-2xl border border-gold/30 bg-[linear-gradient(155deg,rgba(29,47,83,0.55),rgba(10,15,30,0.98)_45%,rgba(7,11,22,0.99)_100%)] shadow-[0_20px_60px_rgba(0,0,0,0.5)]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3.5">
          <h3 className="font-display text-lg tracking-wide text-cream flex items-center gap-2">
            <Swords size={20} className="text-gold" /> Duelo vs {rivalName}
          </h3>
          <button
            onClick={handleClose}
            className="p-1.5 rounded-lg text-muted hover:text-cream hover:bg-surface transition-colors"
          >
            <X size={20} />
          </button>
        </div>

        <div className="p-4 space-y-4">
          {!iHaveDrawn && <DeckStack />}

          <div className="flex items-center justify-center gap-3 sm:gap-5">
            <div className={cn('text-center rounded-xl px-2.5 py-2.5', isResolved && iWon && 'ring-2 ring-green-400/70')}>
              <p className="text-[11px] font-bold text-muted uppercase tracking-wide mb-2">Vos</p>
              <div className="flex gap-2 justify-center">
                <PlayingCardFlip rank={myCard1Rank} suit={myCard1Suit} revealed={myRevealed >= 1} />
                <PlayingCardFlip rank={myCard2Rank} suit={myCard2Suit} revealed={myRevealed >= 2} />
              </div>
              <p className="font-display text-2xl text-cream mt-2">{myFullyRevealed && myTotal !== null ? myTotal : '?'}</p>
            </div>
            <span className="text-gold font-black text-lg">VS</span>
            <div className={cn('text-center rounded-xl px-2.5 py-2.5', isResolved && !iWon && !isTie && 'ring-2 ring-red-400/70')}>
              <p className="text-[11px] font-bold text-muted uppercase tracking-wide mb-2">{rivalName}</p>
              <div className="flex gap-2 justify-center">
                {rivalHasDrawn ? (
                  <>
                    <PlayingCardFlip rank={rivalCard1Rank} suit={rivalCard1Suit} revealed={rivalRevealed >= 1} />
                    <PlayingCardFlip rank={rivalCard2Rank} suit={rivalCard2Suit} revealed={rivalRevealed >= 2} />
                  </>
                ) : (
                  <>
                    <PlayingCardFlip rank={null} suit={null} revealed={false} />
                    <PlayingCardFlip rank={null} suit={null} revealed={false} />
                  </>
                )}
              </div>
              <p className="font-display text-2xl text-cream mt-2">{rivalFullyRevealed && rivalTotal !== null ? rivalTotal : '?'}</p>
            </div>
          </div>

          {error && <p className="text-xs text-red-400 text-center">{error}</p>}

          {isResolved && myFullyRevealed && rivalFullyRevealed && (
            <p
              className={cn(
                'text-center font-display text-lg tracking-wide',
                isTie ? 'text-cream' : iWon ? 'text-green-400' : 'text-red-400'
              )}
            >
              {isTie
                ? '🤝 Empate — nadie gana ni pierde puntos'
                : iWon
                  ? `🏆 ¡Ganaste! Le quitaste 2 puntos a ${rivalName}`
                  : `💀 Perdiste — ${rivalName} te quitó 2 puntos`}
            </p>
          )}

          {!iHaveDrawn && !rivalHasDrawn && (
            <p className="text-xs text-muted text-center">Sos el primero: nadie sacó cartas todavía.</p>
          )}
          {!iHaveDrawn && rivalHasDrawn && (
            <p className="text-xs text-gold text-center">⚔️ {rivalName} ya sacó sus cartas. ¡Es tu turno!</p>
          )}

          {!iHaveDrawn ? (
            <button
              type="button"
              disabled={drawing}
              onClick={handleDraw}
              className="w-full py-3 rounded-xl font-display text-lg tracking-wide bg-gradient-to-b from-gold-2 to-gold text-background shadow-md shadow-gold/20 disabled:opacity-50"
            >
              {drawing ? 'Sacando...' : 'Sacar carta 1'}
            </button>
          ) : myRevealed < 2 ? (
            <button
              type="button"
              onClick={() => setMyRevealed(2)}
              className="w-full py-3 rounded-xl font-display text-lg tracking-wide bg-gradient-to-b from-gold-2 to-gold text-background shadow-md shadow-gold/20"
            >
              Mostrar carta 2
            </button>
          ) : !isResolved ? (
            <div className="rounded-xl border border-border bg-black/20 px-4 py-3 text-center text-sm text-muted">
              ⏳ Esperando que {rivalName} saque sus cartas
            </div>
          ) : rivalRevealed < 2 ? (
            <button
              type="button"
              onClick={() => setRivalRevealed(r => r + 1)}
              className="w-full py-3 rounded-xl font-display text-lg tracking-wide bg-gradient-to-b from-gold-2 to-gold text-background shadow-md shadow-gold/20"
            >
              Mostrar carta {rivalRevealed + 1} de {rivalName}
            </button>
          ) : (
            <button
              type="button"
              onClick={handleClose}
              className="w-full py-3 rounded-xl font-bold text-sm border border-border text-cream hover:border-gold transition-colors"
            >
              Cerrar
            </button>
          )}
        </div>
      </div>
    </div>
  )
}
